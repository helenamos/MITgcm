...
C---+----1----+----2----+----3----+----4----+----5----+----6----+----7-|--+----|
      ...
      Real*8     myTime
      INTEGER myThid
      Real*8  Ptr(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy,PTRACERS_num)
      Real*8     pcbapart_diag(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
      Real*8     pcbppart_diag(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)            
      CHARACTER*8 diagname
      Real*8     TOT_PCB
      Real*8     FRAC_TotDissolved
      Real*8     FRAC_Dissolved
      Real*8     FRAC_POCBound
      Real*8     poco, doco
      INTEGER i,j,k,bi, bj, imin, imax, jmin, jmax
      Real*8     pcba_old, pcbp_old, pcba_new, pcbp_new
      Real*8     Kow   ! octanol-water partitioning coefficient <-- put this in pcb_param.F?


C     == Parameters
C     -- Partition coefficient
c      Real*8 Koc_pcb  ! L/kg
c      data  Koc_pcb /372./ 
        ...
        DO k=1,Nr
         ...
         !---------------------------------------------------  
           ...
           IF (TOT_PCB .gt. 0.D0) THEN
            ...
            !------------------------------------------------ 
c             IF (doco .gt. 4.99D2) THEN ! coast treatment for Hg by yanxu
c               FRAC_DISSOLVED = 1.D0 /
c     &             (1.D0 + Kd_pcbi_poc * 0.2  * poco
c     &             * 1.2D- 8 )  ! mmolC/m3 -> kgC/L
c             ELSE

              FRAC_Dissolved = 1.D0 
     &             /(1.D0 + Koc_pcb * poco 
     &             * 1.2D- 8 )  ! mmolC/m3 -> kgC/L
c             ENDIF
...
C      Slow dissolution or sorption
         ...
         ENDIF !(HFacC(i,j,k,bi,bj).gt.0.D0) 
        ENDDO   !k
       ENDDO    !i
      ENDDO     !j
