...
C
C     Number of tracers
c     - Changed from 3 to 2 (hma, 15 Jul 2014)
...
C     myThid     :: my Thread Id number
      Real*8  PTR (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr, 
     &             nSx, nSy, PTRACERS_num)
      Real*8  SurfPCBa (1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  FluxPCBa(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
c declared in PFC.h
c      Real*8  PCBaRiver(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  pcbarivero, pcbaevaso
c declared in PFC.h
c      Real*8  PCBaAtmDep(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy) ! atmospheric deposition
      Real*8  pcbadepoo                                        ! atmospheric deposition
      Real*8 myTime
...
C Solubility relation coefficients
      ...
      Real*8 SchmidtAPCBa
      Real*8 SchmidtWPCBa
      Real*8 DiffusivityAir(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 DiffusivitySurfWater(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 ka(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 kw(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 Kaexch(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
c      Real*8 Kwexch(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      
      Real*8 KawPCBa(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 KawPCBeff(1-OLx:sNx+OLx,1-OLy:sNy+OLy)

c     Real*8 pcb01sat(1-OLx:sNx+OLx,1-OLy:sNy+OLy)

c      Real*8 Csat(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 ttemp, ttemp2
      Real*8 temp1, temp2, temp3, temp4, temp5, temp6
      Real*8 doco
      Real*8 C2K      ! conversion factor, Celcius to Kelvin (hma, 22 jul 2014)
      Real*8 R        ! universal gas constant (hma, 22 jul 2014)
      Real*8 TK0      ! temperature at standard conditions (hma, 22 jul 2014)
      Real*8 KH       ! liquid-over-gas Henry's law constant (hma, 22 jul 2014)
      Real*8 KHcc     ! dimensionless liquid-over-gas Henry's law constant (hma, 22 jul 2014)
      Real*8 KawD     ! dimensionless gas-over-liquid Henry's law constant (hma, 22 jul 2014)
      Real*8 eta_s    ! dynamic viscocity of water 
      Real*8 Ms       ! relative molecular mass of water
      Real*8 PHI      ! association factor of water
      Real*8 pcbaconc_diag(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 pcbaevas_diag(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8 pcbariver_diag(1-OLx:sNx+OLx,1-OLy:sNy+OLy) ! river inputs
      Real*8 pcbaconco                                   ! atmospheric concentration?
      Real*8 pcbadep_diag(1-OLx:sNx+OLx,1-OLy:sNy+OLy)   ! atmospheric deposition
...
c            Kaexch (i,j) = (1.D0 - fIce(i,j,bi,bj))
c     &   /(1.d0/ka(i, j)+KawPCBeff(i,j)/kw(i,j)) 
             Kaexch (i,j) = (1.D0 - fIce(i,j,bi,bj))
...
C        pcbaseasprayo =  (1.D0 - fIce(i,j,bi,bj))
...
c$$$C---------------------------------------------------            
c$$$C Calculate diagnistics
c$$$C--------------------------------------------------- 
c$$$#ifdef 
c$$$            ! Air concentration (ng m^-3)
c$$$            pcbaconc_diag(i,j)  = pcbaconco
c$$$ 
c$$$            ! From rivers (mol m^-2 sec^-1)
c$$$            pcbariver_diag(i,j) = pcbarivero
c$$$
c$$$            ! Ocean-to-air evasion (mol m^-2 sec^-1) 
c$$$            pcbaevas_diag(i,j)  = pcbaevaso  
c$$$
c$$$            ! Inputs from atmospheric deposition (mol m^-2 sec^-1)
c$$$            pcbadep_diag(i,j)   = pcbadepoo
c$$$
c$$$c Disable sea spray (hma, 21 jul 2014)
c$$$c#ifdef ALLOW_SEASPRAY
c$$$c            pcbaseaspray_diag(i,j) = pcbaseasprayo
c$$$c#endif          
c$$$
c$$$#endif       
