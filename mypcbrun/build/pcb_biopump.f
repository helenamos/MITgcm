...
C     == Routine arguments ==
      ...
      Real*8  Gchem(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy,PTRACERS_num)    
      Real*8  pcbpsink_diag(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
...
c	      INTEGER nriver
c	      Real*8  whgr(nrmax)
c	      data whgr /100.0D0, 10.0D0, 1.0D0/
c	#if nrmax > 3
c	      What is the sinking speed of riverine particles?
c	#endif             
c	      Real*8  Fout_Hgr(Nr,nrmax)
c	#ifdef       
c	      Real*8  hgrsink_diag(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
c	#endif      
c	#endif        
c #ifdef METHY_MODE       
c      Real*8  Fout_MMHgp(Nr)
c #ifdef       
c      Real*8  mmhgpsink_diag(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr)
c #endif      
c #endif      
