...
C     == Routine arguments ==
      ...
      Real*8  surfpcbp(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  Fluxpcbp(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8  pcbprivero ! riverine discharge
      Real*8  pcbpdepoo   ! atmospheric deposition
      Real*8  pcbpriver_diag(1-OLx:sNx+OLx,1-OLy:sNy+OLy) ! riverine discharge
      Real*8  pcbpdep_diag(1-OLx:sNx+OLx,1-OLy:sNy+OLy)   ! atmospheric deposition
...
C Initialize diagnostics
     ...
     &             *recip_rA(i,j,bi,bj)*1000.d0/Mb
             ...
             ! Inputs from  atmospheric deposition (should be mol/m2/s)
             ! ** What are the current units of PCBpDEP? 
             pcbpdepoo = PCBpDEP(i,j,bi,bj) ! x conversion factors

             ! Total particulate PCB inputs (mol/m2/s)
             !Fluxpcbp(i,j) = pcbprivero
             Fluxpcbp(i,j) = pcbpdepoo
