...
c   Forcing files
     ...
     &       offlineLoadPrec
      Real*8 ICEM(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 ConvectCount(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nR,nSx,nSy)
      Real*8 deltaToffline
      Real*8 offlineForcingPeriod
      Real*8 offlineForcingCycle
...
C     aWght, bWght :: Interpolation weights
     ...
     &                 sflx1, kdfs1, kght1, icem1
      Real*8  uvel0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  uvel1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  vvel0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  vvel1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  wvel0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  wvel1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  tave0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  tave1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  save0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  save1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  conv0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  conv1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  gmkx0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  gmkx1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  gmky0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  gmky1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  gmkz0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  gmkz1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  hflx0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  hflx1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  sflx0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  sflx1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  icem0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  icem1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8  kdfs0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  kdfs1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  kght0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  kght1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      ...
      DO bj = myByLo(myThid), myByHi(myThid)
        DO bi = myBxLo(myThid), myBxHi(myThid)
          ...
          DO k=1,Nr
            DO j=1-Oly,sNy+OLy
              DO i=1-Olx,sNx+Olx
                 save0(i,j,k,bi,bj) = 0.D0
                 tave0(i,j,k,bi,bj) = 0.D0
                 uvel0(i,j,k,bi,bj) = 0.D0
                 vvel0(i,j,k,bi,bj) = 0.D0
                 wvel0(i,j,k,bi,bj) = 0.D0
                 conv0(i,j,k,bi,bj) = 0.D0
                 gmkx0(i,j,k,bi,bj) = 0.D0
                 gmky0(i,j,k,bi,bj) = 0.D0
                 gmkz0(i,j,k,bi,bj) = 0.D0
                 kdfs0(i,j,k,bi,bj) = 0.D0
                 kght0(i,j,k,bi,bj) = 0.D0
C
                 save1(i,j,k,bi,bj) = 0.D0
                 tave1(i,j,k,bi,bj) = 0.D0
                 uvel1(i,j,k,bi,bj) = 0.D0
                 vvel1(i,j,k,bi,bj) = 0.D0
                 wvel1(i,j,k,bi,bj) = 0.D0
                 conv1(i,j,k,bi,bj) = 0.D0
                 gmkx1(i,j,k,bi,bj) = 0.D0
                 gmky1(i,j,k,bi,bj) = 0.D0
                 gmkz1(i,j,k,bi,bj) = 0.D0
                 kdfs1(i,j,k,bi,bj) = 0.D0
                 kght1(i,j,k,bi,bj) = 0.D0
              ENDDO
            ENDDO
          ENDDO
          DO j=1-Oly,sNy+OLy
            DO i=1-Olx,sNx+Olx
                 hflx0(i,j,bi,bj) = 0.D0
                 sflx0(i,j,bi,bj) = 0.D0
                 icem0(i,j,bi,bj) = 0.D0
C
                 hflx1(i,j,bi,bj) = 0.D0
                 sflx1(i,j,bi,bj) = 0.D0
                 icem1(i,j,bi,bj) = 0.D0
            ENDDO
          ENDDO
        ENDDO
      ENDDO
