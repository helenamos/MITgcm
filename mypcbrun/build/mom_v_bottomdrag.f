...
C  i,j                  :: loop indices
      ...
      Real*8 recDrC
      Real*8 recDrF_bot(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
CEOP
...
c      dragFac = wUnit2rVel(k)
      ...
      IF ( k.EQ.kBottom ) THEN
       ...
       DO j=1-OLy,sNy+OLy
        DO i=1-OLx,sNx+OLx
          recDrF_bot(i,j) = recip_hFacS(i,j,k,bi,bj)*recip_drF(k)
        ENDDO
       ENDDO
      ELSE
...
C-    Add quadratic bottom drag
      IF ( bottomDragQuadratic.NE.0.D0 ) THEN
       DO j=1-OLy+1,sNy+OLy-1
        DO i=1-OLx,sNx+OLx-1
