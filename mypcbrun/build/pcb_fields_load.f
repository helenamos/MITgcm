...
c       INTEGER bi, bj, i, j
c      Real*8 aWght, bWght
c      Real*8 locWind(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
CEOP
c=================================
...
c===========from hg code=========
      ...
      IF ( PCB_forcingCycle .GT. 0.D0 ) THEN
...
c         CALL EXCH_XY_RS ( wind0,  myThid  )
c         CALL EXCH_XY_RS ( wind1,  myThid  )
c        ENDIF
c         CALL EXCH_XY_RS ( atmosp0,  myThid  )
c         CALL EXCH_XY_RS ( atmosp1,  myThid  )
c        IF ( .NOT.useSEAICE .AND. .NOT.useThSIce ) THEN
c         CALL EXCH_XY_RS ( ice0,  myThid  )
c         CALL EXCH_XY_RS ( ice1,  myThid  )
c        ENDIF
c============================        

         CALL EXCH_XY_RS ( wind0,  myThid  )
         CALL EXCH_XY_RS ( wind1,  myThid  )
...
c	#endif        
     ...
     &        myIter,myThid )
        ENDIF
         CALL EXCH_XY_RS ( radsw0,  myThid  )
         CALL EXCH_XY_RS ( radsw1,  myThid  )
        IF (.NOT.useSEAICE) THEN
         CALL EXCH_XY_RS ( ice0,  myThid  )
         CALL EXCH_XY_RS ( ice1,  myThid  )
        ENDIF
C         CALL EXCH_XY_RS ( pcbaconc0,  myThid  )
C         CALL EXCH_XY_RS ( pcbaconc1,  myThid  )
C         CALL EXCH_XY_RS ( pcbidepo0,  myThid  )
C         CALL EXCH_XY_RS ( pcbidepo1,  myThid  )
         CALL EXCH_XY_RS ( pcbadepo0,  myThid  ) ! PCBa atmopsheric deposition
         CALL EXCH_XY_RS ( pcbadepo1,  myThid  ) ! PCBa atmospheric deposition 
         CALL EXCH_XY_RS ( pcbpdepo0,  myThid  ) ! PCBp atmospheric deposition
         CALL EXCH_XY_RS ( pcbpdepo1,  myThid  ) ! PCBp atmospheric deposition
C         CALL EXCH_XY_RS ( pcbiriver0,  myThid  )
C         CALL EXCH_XY_RS ( pcbiriver1,  myThid  )
C         CALL EXCH_XY_RS ( pcbpriver0,  myThid  )
C         CALL EXCH_XY_RS ( pcbpriver1,  myThid  ) 
c #ifdef HGPRIVER
c         DO nriver=1,nrmax
c          CALL EXCH_XY_RS ( hgrriver0(1-OLx,1-OLy,1,1,nriver), myThid )
c          CALL EXCH_XY_RS ( hgrriver1(1-OLx,1-OLy,1,1,nriver), myThid )
c         ENDDO 
c#endif                                  
         CALL EXCH_XYZ_RS ( chl0,  myThid  )
         CALL EXCH_XYZ_RS ( chl1,  myThid  )
         CALL EXCH_XYZ_RS ( npp0,  myThid  )
         CALL EXCH_XYZ_RS ( npp1,  myThid  )
         CALL EXCH_XYZ_RS ( doc0,  myThid  )
         CALL EXCH_XYZ_RS ( doc1,  myThid  )
         CALL EXCH_XYZ_RS ( poc0,  myThid  )
         CALL EXCH_XYZ_RS ( poc1,  myThid  )
         CALL EXCH_XYZ_RS ( Fdoc0,  myThid  )
         CALL EXCH_XYZ_RS ( Fdoc1,  myThid  )
         CALL EXCH_XYZ_RS ( Fpoc0,  myThid  )
         CALL EXCH_XYZ_RS ( Fpoc1,  myThid  )
         CALL EXCH_XYZ_RS ( Fremin0,  myThid  )
         CALL EXCH_XYZ_RS ( Fremin1,  myThid  )
C-    save newly loaded time-record
        DO bj = myByLo(myThid), myByHi(myThid)
         DO bi = myBxLo(myThid), myBxHi(myThid)
...
c            ELSE
c              HgrRiver(i,j,bi,bj,nriver) = 0.D0
c            ENDIF 
c           ENDDO
c #endif            
C          ENDDO
...
C endif for periodicChem !for IF ( PCB_chemCycle .GT. 0.D0 ) THEN
      ENDIF 
...
c        ENDIF
     ...
     &       myIter,myThid )
        ENDIF
         ...
         CALL EXCH_XY_RS ( pcbaconc0,  myThid  )
         CALL EXCH_XY_RS ( pcbaconc1,  myThid  )
c Remove PCBi tracer. (hma, 15 Jul 2014)
c         CALL EXCH_XY_RS ( pcbidepo0,  myThid  )
c         CALL EXCH_XY_RS ( pcbidepo1,  myThid  )
c         CALL EXCH_XY_RS ( pcbiriver0,  myThid  )
c         CALL EXCH_XY_RS ( pcbiriver1,  myThid  )
         CALL EXCH_XY_RS ( pcbpriver0,  myThid  )
         CALL EXCH_XY_RS ( pcbpriver1,  myThid  ) 
...
C endif for periodicEmis ! of IF ( PCB_emisCycle .GT. 0.D0 ) 
      ENDIF 
