...
cph)
C
      Real*8 xoamc, yoamc, zoamc, xoamp, yoamp, zoamp
      Real*8 mass, xcom, ycom, zcom
      Real*8 sbobp, sboarea, sboempmrwet, sboqnetwet
      Real*8 obp (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C----------------------------------------------------------------
      ...
      Real*8 OBPtave (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
...
C     sbo_omega :: earth s mean angular velocity (rad/s)
      ...
      Real*8 lat, lat_deg, lon, radius, darea, dradius, dvolume, depth
      Real*8 u, v, density
      Real*8 ae, grav, sbo_omega
      PARAMETER ( ae        = 6.3710D6    )
      PARAMETER ( grav      = 9.8156         )
      PARAMETER ( sbo_omega = 7.292115D-5 )
CEOP
...
cph              t = theta(i,j,kn0,bi,bj)
              u =(uvel(i,j,kn0,bi,bj)+uvel(i+1,j,kn0,bi,bj))*0.5D0
              v =(vvel(i,j,kn0,bi,bj)+vvel(i,j+1,kn0,bi,bj))*0.5D0
...
C- note: GLOBAL_SUM applied to var. in common block <= wrong if Muti-threaded
      CALL GLOBAL_SUM_R8 (  mass  ,  myThid  )
      CALL GLOBAL_SUM_R8 (  xcom  ,  myThid  )
      CALL GLOBAL_SUM_R8 (  ycom  ,  myThid  )
      CALL GLOBAL_SUM_R8 (  zcom  ,  myThid  )
      CALL GLOBAL_SUM_R8 (  xoamc ,  myThid  )
      CALL GLOBAL_SUM_R8 (  yoamc ,  myThid  )
      CALL GLOBAL_SUM_R8 (  zoamc ,  myThid  )
      CALL GLOBAL_SUM_R8 (  xoamp ,  myThid  )
      CALL GLOBAL_SUM_R8 (  yoamp ,  myThid  )
      CALL GLOBAL_SUM_R8 (  zoamp ,  myThid  )
cph(
      CALL GLOBAL_SUM_R8 (  sbobp,  myThid  )
      CALL GLOBAL_SUM_R8 (  sboarea,  myThid  )
      CALL GLOBAL_SUM_R8 (  sboempmrwet,  myThid  )
      CALL GLOBAL_SUM_R8 (  sboqnetwet,  myThid  )
cph)
