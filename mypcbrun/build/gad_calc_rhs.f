C $Header: /u/gcmpack/MITgcm/pkg/generic_advdiff/gad_calc_rhs.F,v 1.62 2012/07/04 20:21:45 jmc Exp $
...
C locABT           :: local copy of (AB-extrapolated) tracer field
      ...
      Real*8 outFlux, trac, fac, gTrFac
...
coj   Add outgoing fluxes
     ...
     &    *( MAX(0.D0,fZon(i+1,j)) + MAX(0.D0,-fZon(i,j))
     &      +MAX(0.D0,fMer(i,j+1)) + MAX(0.D0,-fMer(i,j))
     &      +MAX(0.D0,fVerT(i,j,kDown)*rkSign)
     &      +MAX(0.D0,-fVerT(i,j,kUp)*rkSign)
     &     )
         IF ( applyAB_onTracer ) THEN
...
coj   SmolarkiewiczMaxFrac, scale them down
         IF (outFlux.GT.0.D0 .AND.
     &       outFlux.GT.SmolarkiewiczMaxFrac*trac) THEN
coj   If tracer is already negative, scale flux to zero
           fac = MAX(0.D0,SmolarkiewiczMaxFrac*trac/outFlux)

           IF (fZon(i+1,j).GT.0.D0) fZon(i+1,j)=fac*fZon(i+1,j)
           IF (-fZon(i,j) .GT.0.D0) fZon(i,j)  =fac*fZon(i,j)
           IF (fMer(i,j+1).GT.0.D0) fMer(i,j+1)=fac*fMer(i,j+1)
           IF (-fMer(i,j) .GT.0.D0) fMer(i,j)  =fac*fMer(i,j)
           IF (-fVerT(i,j,kUp)*rkSign .GT.0.D0)
     &       fVerT(i,j,kUp)=fac*fVerT(i,j,kUp)

           IF (k.LT.Nr .AND. fVerT(i,j,kDown)*rkSign.GT.0.D0) THEN
